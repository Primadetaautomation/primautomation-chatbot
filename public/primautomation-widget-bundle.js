(() => {
  ...
  var API_BASE = window.PRIMAUTOMATION_API_URL || "";
  ...
  function renderMarkdown(text) {
    ...
    return esc.replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/__(.+?)__/g, "<strong>$1</strong>").replace(/(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)/g, "<em>$1</em>").replace(/^[\-\u2022]\s+(.+)$/gm, "<li>$1</li>").replace(/((?:<li>.*<\/li>\n?)+)/g, "<ul>$1</ul>").replace(/^\d+\.\s+(.+)$/gm, "<li>$1</li>").replace(/((?:<li>.*<\/li>\n?)+)/g, (m) => m.includes("<ul>") ? m : `<ol>${m}</ol>`).replace(/\n/g, "<br>").replace(/<br><li>/g, "<li>").replace(/<\/li><br>/g, "</li>");
  }
  ...
  async function submitContactForm() {
    ...
    function isValidPhone(n) {
      return /^(\+|0)\d{7,14}$/.test(n.replace(/[\s\-\(\)\.]/g, ""));
    }
  ...
  function startPolling() {
    ...
    let lastPollTime = (/* @__PURE__ */ new Date()).toISOString();
  ...
  function wireEvents() {
    ...
    document.addEventListener("click", (e) => {
      if (state.isOpen && !dom.panel.contains(e.target) && !dom.bubble.contains(e.target) && window.innerWidth > 768) {
        closePanel();
      }
    });
